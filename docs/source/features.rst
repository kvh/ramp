...
1. Features attempt to store everything they compute for later reuse. They
...
2. Features may depend on target "y" values (Feature selectors for instance).
   These features will only be built with values in the given
   `DataContext`'s train_index.
3. Similarly, Features may depend on other "x" valeus.
...
4. Feature instances should not store state (except temporarily while being created they
